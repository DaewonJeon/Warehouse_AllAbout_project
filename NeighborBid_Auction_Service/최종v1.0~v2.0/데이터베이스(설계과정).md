...
|:---:|---|:---:|:---:|:---:|
| 전국 실시간 | 한정판 신발, 인기 굿즈 | 초당 10~100건 |  **필수** | 높음 (WebSocket + Redis) |
...
*   **동작 방식 예시**: 환불 발생 시 기존 내역을 지우는 것이 아니라, `BID_REFUND`라는 새로운 타입의 **트랜잭션(Compensating Transaction)**을 생성하여 기록을 남깁니다.
...
정규화 원칙에 맞춰 매번 `Bid` 테이블을 조회하여 `MAX(amount)`를 계산 
=> **이는 조회의 빈도가 압도적으로 높은 서비스 특성상 비효율적이라고 판단** 했습니다.

#### 정규화 vs 반정규화 트레이드오프

| 방식 | 쿼리 예시 | 성능 | 데이터 정합성 |
|:---:|---|:---:|:---:|
| 정규화 | `SELECT MAX(amount) FROM bid WHERE auction_id = ?` |  O(N) |  항상 정확 |
| 반정규화 | `SELECT current_price FROM auction WHERE id = ?` |  O(1) |  갱신 로직 필요 |

저희는 **반정규화(Denormalization)** 방식을 택하여 `Auction` 테이블에 `current_price` 컬럼을 두고, 입찰이 발생할 때마다 갱신하는 전략을 사용했습니다. 이 방식은 쓰기 시 약간의 오버헤드가 발생하지만, 조회가 훨씬 빈번한 경매 시스템의 특성(Read:Write 비율 100:1 이상)상 읽기 성능을 **O(1)**로 최적화하는 것이 합리적이기 때문입니다.

#### 실제 구현 (Implementation)

**1. 쓰기 비용의 희생 (`auctions/services.py`)**
입찰 시 `Bid` 테이블에 insert하는 것 외에, `Auction` 테이블의 `current_price`를 업데이트하는 추가 쿼리가 발생합니다.

```python
# auctions/services.py (place_bid 함수 내부)
with transaction.atomic():
    # ... (입찰 유효성 검증 및 Bid 생성) ...

    # [Write Overhead] 조회 성능을 위해 부모 테이블도 갱신
    auction.current_price = amount
    auction.save()
```

**2. 읽기 성능의 이득 (`auctions/views.py`)**
덕분에 리스트 조회나 필터링 시에는 무거운 집계 연산(Aggregation) 없이 단순 `SELECT`만으로 처리할 수 있습니다.

```python
# auctions/views.py
def auction_list(request):
    # JOIN, MAX 연산 없이 바로 필터링 가능 (O(1))
    if min_price:
...
**3. 데이터 정합성 보장 (Consistency Guarantee)**

*   **Atomic Transaction**: 입찰 내역 생성(Insert)과 현재가 갱신(Update)을 하나의 트랜잭션으로 묶어, 둘 중 하나라도 실패하면 전체 롤백됩니다.
*   **Pessimistic Lock**: `select_for_update`를 통해 동시 다발적인 쓰기 요청을 순차적으로 처리(Serialize)하여, 경쟁 상태에서도 데이터가 꼬이는 것을 원천 차단했습니다.
