...
- Visualization Tool: pgAdmin 4 Dashboard (TPS, Tuples Fetched, Active Sessions 모니터링)
...
- PostgreSQL 지표 분석:
  - Tuples Fetched 급증: 인덱스 부재 시, 최신값 1개를 찾기 위해 전체 파티션을 스캔함.
...
- **📊 Dashboard**: 
  - `Tuples Fetched` 수치 폭발 (비효율적 I/O)
  - `Active Sessions`가 줄어들지 않고 적체됨
...
- **📊 Dashboard**: 
  ...
  - `Tuples Fetched`가 조회된 행의 개수와 거의 일치 (I/O 낭비 0).
...
2. 비정규화는 조회 비용을 '0'으로 만든다: 
   - 대시보드 지표에서 `CPU`와 `Tuples Fetched`가 바닥을 치는 것을 확인.
